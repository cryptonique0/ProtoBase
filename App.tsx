...
import { getOrCreateProfile } from './src/services/profileService';
...
const WalletModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  const walletIcons: Record<string, React.FC> = {
    ...
    walletConnect: WalletIcons.Zero,
  };
      ...
      <div className="bg-surface-dark border border-border-dark rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-scaleUp">
        ...
        <div className="p-6 space-y-3">
          {connectors.map((connector) => {
            const Icon = walletIcons[connector.id] || WalletIcons.MetaMask;
            return (
              <button
                key={connector.id}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-bold text-gray-200 group-hover:text-white">{connector.name}</span>
                </div>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isConnected && address) {
      getOrCreateProfile(address)
        .then(profile => {
          setUser(profile.id, address, profile);
        })
        .catch(err => {
  ...
  // Switch to Base network when connected
  useEffect(() => {
  ...
  const renderPage = () => {
    switch (currentStep) {
      ...
      case ProjectStep.DEPLOYMENT:
        return <DeploymentPage enableRealDeployment={true} isGenerating={isGenerating} sourceCode={generatedSource} onComplete={() => navigateTo(ProjectStep.MONITOR)} onBack={() => navigateTo(ProjectStep.FRONTEND)} />;
      ...
      case ProjectStep.IDEAS:
        return <IdeasListPage onSelectIdea={(ideaId) => {
          setSelectedIdeaId(ideaId);
          navigateTo(ProjectStep.IDEA_DETAIL);
        }} />;
      case ProjectStep.IDEA_DETAIL:
    ...
    <div className="min-h-screen bg-black text-white flex flex-col font-display selection:bg-primary/40">
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-border-dark bg-black/80 backdrop-blur-md px-6 py-4">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <button onClick={() => navigateTo(ProjectStep.IDEAS)} className="text-sm font-bold text-text-secondary hover:text-white transition-colors">Ideas</button>
        ...
        <div className="flex items-center gap-4 lg:gap-6">
          ...
          {chain && chain.id !== base.id && isConnected && (
            <button
              onClick={() => switchChain?.({ chainId: base.id })}
              className="px-3 py-1.5 rounded-lg text-xs font-bold bg-yellow-500/10 border border-yellow-500/50 text-yellow-500 hover:bg-yellow-500/20 transition-all"
            >
              Switch to Base
            </button>
          )}
          <button 
            ...
            className={`px-4 py-2 lg:px-5 lg:py-2.5 rounded-xl text-xs font-bold border transition-all ${isConnected ? 'bg-primary/10 border-primary/50 text-primary' : 'bg-surface-dark border-border-dark text-white hover:border-gray-500'}`}
          >

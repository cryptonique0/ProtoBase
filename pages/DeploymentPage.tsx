...
const DeploymentPage: React.FC<Props> = ({ onComplete, onBack, sourceCode, isGenerating }) => {
  ...
  // Memoize random values for the session
  ...
  useEffect(() => {
    ...
    if (sourceCode && !done && logs.length < DEPLOYMENT_STEPS.length + 1) {
      let stepIndex = 0;
